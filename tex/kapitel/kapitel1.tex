Einführung
    Motivation
        Warum (Fallbeispiele)
        Aktuelle Entwicklung (Digitalisierung, Industrie4.0)
    Zweck
    ...
    Struktur
        Aufbau der Arbeit
Verwandte Arbeit
    Zwei drei Arbeiten die auch über MQTT Proxies bzw MQTT (IoT) Pentesting / Security Assessment z.B.: OWASP IoT Guide, diverse Github implementierungen
Grundlagen
    ...
    MQTT Protokol
    MITM
    Funktionalität eines Proxies
    Security Assessment (OWASP IoT Guide)
Konzept
    MITM (APR Spoof, ICMP ,FW)
    Versuch1: Scapy
    Versuch2: FW
Implementierung (Spezielle Herausforderungen)
    ?
Zusammenfassung
    Zweck->Konzept->Thesen
    Validieren der Thesen
    Ergebnisse
    Erkenntnisse
Fazit
    Ausblick
