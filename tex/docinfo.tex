...
Weitere Probleme, neben den Eigenschaften des Protokolls, sind die fehlerhafte Konfiguration und Einrichtung sowie die intransparente Kommunikation der Produkte. Es entstehen nicht nur Schwachstellen die gefunden werden müssen, sondern auch Datenübertragungen die verhindert werden sollten, um den Abfluss von privaten Daten zu verhindern.

%Ziel
Die Arbeit hat zum Ziel, eine Software zu konzipieren und zu entwickeln, die es möglich macht, die Kommunikation zwischen \acs{IoT}-Geräten und deren Betreibern abzufangen und zu modifizieren.
%Was in der Arbeit gemacht wurde
Um Unterstützung beim Überprüfen solcher Geräte zu erhalten, wird ein Konzept entwickelt, welches nicht nur unabhängig von der Plattform, sondern auch von der Programmiersprache und dem untersuchten Protokoll ist.
%Ergebnisse
...
\newcommand{\hsmaabstracten}{More and more intelligent devices are used in the domestic environment and industry. This is partially due to the reduction of operating costs and the convenience that users experience using them. Often, these devices use a protocol that does not guarantee security by design, therefore, manufacturers must implement security mechanisms on their own.
Aside from the characteristics of the protocol, additional issues are the misconfiguration and improper setup of the products as well as the intrasparent communication. Not only vulnerabilities need to be found, but also data transfers that should be blocked to prevent the outflow of private data need to be identified.

%Ziel
The objective of this bachelor thesis is to design a concept and implement a program with the ability to intercept the communication between \acs{IoT} devices and their targets.
%Was in der Arbeit gemacht wurde
...
%Ergebnisse
Afterwards, this concept, with support for the \acs{MQTT}-protocol, is implemented and tested in a virtual laboratory. The results show that the software is capable of intercepting and changing communication. Using various features, such as modifying the messages, resending, or rebuilding them, it is possible to test the communication and endpoints.}